    ...
    public class RssService : IRssService
    {
        ...
        public async Task<HttpResponseMessage> Import(RssImportModel model)
        {
            ...
            try
            {
                ...
                Logger.LogInformation(string.Format("Start importing {0} posts", items.Count));
                ...
                foreach (var item in items)
                {
                    ...
                    _db.Complete();
                    Logger.LogInformation(string.Format("RSS item added : {0}", item.Title));
            ...
            {
                Logger.LogError(string.Format("Error importing RSS : {0}", ex.Message));
        ...
        IList<FeedItem> GetFeedItems(string url)
        {
            ...
            Logger.LogInformation("Importing RSS from feed: " + url);
        ...
        async Task AddAssets(IList<string> assets, BlogPost post, IBlogStorage storage, bool isAttachement)
        {
            if (assets.Any())
            {
                foreach (var item in assets)
                {
                    ...
                    {
                        Logger.LogError(string.Format("Error importing [{0}] : {1}", item, ex.Message));
                    }
                }
            }
        }
